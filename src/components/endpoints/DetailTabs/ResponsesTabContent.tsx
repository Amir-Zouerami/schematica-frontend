	...
	useEffect(() => {
		...
		if (responses) {
			Object.entries(responses).forEach(
				([statusCode, responseItemUntyped], responseIndex) => {
				...
				if (
					...
					return (
						<div
							...
							className="p-4 border border-border rounded-lg text-muted-foreground"
						>
					...
					>
						<CollapsibleTrigger className="flex items-center justify-between w-full bg-card px-4 py-3 hover:bg-secondary/30 transition-colors rounded-t-lg data-[state=open]:border-b data-[state=open]:border-border">
							<div className="flex items-center">
								<span
									className={`font-mono text-lg font-bold ${statusCodeColorClass} mr-3`}
								>
									{statusCode}
								</span>
						...
						<CollapsibleContent className="p-4 bg-background/50 rounded-b-lg">
							...
							Object.keys(responseItem.content).length > 0 ? (
									Object.entries(responseItem.content).map(
										([contentType, mediaTypeUntyped], ctIndex) => {
											const mediaType = mediaTypeUntyped as MediaTypeObject;
											const mediaTypeCollapsibleId = `response-${statusCode}-${contentType}-${ctIndex}`;
											const isMediaTypeOpen =
												!!openCollapsibles[mediaTypeCollapsibleId];
