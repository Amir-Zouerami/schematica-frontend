import { ParsedCurlCommand, toJsonObject } from '@/shared/lib/curlconverter-wrapper';
import { isRefObject, resolveRef } from '@/shared/lib/schemaUtils';
...
interface ExampleGenerationContext {
	fieldName?: string;
	isPreResolved?: boolean;
	parentContentType?: string;
}

export const generateExampleValue = (
	schemaOrPreResolvedExample: SchemaObject | ReferenceObject | unknown,
	...
	if (!schemaOrPreResolvedExample || typeof schemaOrPreResolvedExample !== 'object') {
		return null;
	}
	...
	if (isRefObject(schema) && openApiSpec) {
		...
		if (resolvedSchema && typeof resolvedSchema === 'object' && !isRefObject(resolvedSchema)) {
...
export const convertOpenApiToCurl = (
	...
	const resolveAndAddParam = (paramOrRef: ParameterObject | ReferenceObject) => {
		...
		if (isRefObject(paramOrRef)) {
			...
			if (resolved && typeof resolved === 'object' && !isRefObject(resolved)) {
	...
	if (operation.requestBody) {
		...
		if (isRefObject(operation.requestBody)) {
			...
			if (
				resolvedReqBody &&
				typeof resolvedReqBody === 'object' &&
				!isRefObject(resolvedReqBody)
			)
					...
					if (actualContentType.includes('json')) {
						...
						for (const key in exampleData) {
							if (Object.prototype.hasOwnProperty.call(exampleData, key)) {
								const val = (exampleData as Record<string, unknown>)[key];
								formDataParts.push(`${key}=${val}`);
							}
						}

						requestBodyDataString = formDataParts.join('&');
					} else if (typeof exampleData === 'string') {
						if (!explicitlySetContentType) {
							headersToAdd['Content-Type'] = chosenContentType || 'text/plain';
						}
						requestBodyDataString = exampleData;
					} else {
						if (!explicitlySetContentType) {
							headersToAdd['Content-Type'] = chosenContentType || 'application/json';
						}
						requestBodyDataString = JSON.stringify(exampleData, null, 2);
					}
				}
...
export const inferSchemaFromValue = (value: unknown): SchemaObject => {
	...
	if (type === 'string') {
		...
		if (/^\d{4}-\d{2}-\d{2}$/.test(strVal)) return { type: 'string', format: 'date' };
		if (
		...
		if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(strVal)) return { type: 'string', format: 'email' };
		return { type: 'string' };
	}
	...
	try {
		...
		if (parsedCurl.queries) {
			for (const key in parsedCurl.queries) {
				if (Object.prototype.hasOwnProperty.call(parsedCurl.queries, key)) {
		...
		if (parsedCurl.headers) {
			for (const key in parsedCurl.headers) {
				if (Object.prototype.hasOwnProperty.call(parsedCurl.headers, key)) {
		...
		const operation: components['schemas']['OpenApiOperationDto'] = {
			...
			parameters:
				parameters.length > 0
					? (parameters as unknown as components['schemas']['ParameterObject'][])
					: undefined,
			requestBody: requestBody as unknown as components['schemas']['RequestBodyObject'],
					...
					if (targetContent) {
						...
						if (
							...
							if (isRefObject(firstExampleOrRef)) {
								...
								exampleForPostman = (
									resolvedExampleComponent as ExampleObject | null
								)?.value;
							} else {
